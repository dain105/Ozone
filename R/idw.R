...
gp.mean<-read.csv("S:/Users/Dain/gp.mean.csv")
gp.mean<-gp.mean[,2:34]
ar.mean<-read.csv("S:/Users/Dain/AR.mean.csv")
ar.mean<-ar.mean[,2:34]
...
for(i in 1:33){
new.data[((552*i)-551):(552*i),3]<-ar.mean[,i]
}
new.data<-as.data.frame(new.data)
str(new.data)
colnames(new.data)<-c("year","air.station","o3")
air.lo<-read.csv("S:/Users/Dain/lon.csv",head=T)
...
min_<-min(minValue(r))
max_<-max(maxValue(r))
breaks <- seq(35, 60, by=0.1)

cols <- colorRampPalette(c( "lightgreen","green", "Yellow","Orange","red"))(length(breaks)-1)



p<-plot(r,xlim=c(126,131),ylim=c(34,40),cex.main=0.8,col=cols,breaks=breaks,legend=F)
