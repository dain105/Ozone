...
coords <-as.matrix(o3.fit[,16:17])#asos
dim(coords)
...
o3.val<-cbind(data$asos.lon,data$asos.lat,data$o3)
colnames(o3.val)<-c("lon","lat","o3")
o3.val<-o3.val[is.na(o3.val[,3]),1:2]
pred.coords <- o3.val[,1:2]
pred.covars <- cbind(rep(1,dim(o3.val)[1]),o3.val)
# new.data<-cbind(data$air.lon,data$air.lat,data$o3)
# colnames(new.data)<-c("air.lon","air.lat","o3")


predicted <-spPredict(m.1, pred.covars=pred.covars, pred.coords=pred.coords)
o3<-rep(0,dim(predicted$p.y.predictive.samples)[1])
for (j in 1:dim(predicted$p.y.predictive.samples)[1]){
  o3[j]<-quantile(predicted$p.y.predictive.samples[j,],c(0.5))}

replaced.o3<-cbind(data[is.na(data[,18]),1:17],o3)
new.data<-data[!is.na(data$o3),]
replaced.o3.data<-rbind(replaced.o3,new.data)
replaced.o3.data<-replaced.o3.data[order(replaced.o3.data[,3],replaced.o3.data[,2]), ]
replaced.o3.data<-replaced.o3.data[,2:18]

setwd("S:/Users/Dain/연구/졸업논문/data/final data")
write.csv(replaced.o3.data, "replaced.o3.data.csv", append = FALSE)
